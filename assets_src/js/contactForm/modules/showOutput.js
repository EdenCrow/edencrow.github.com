function input(errors, classes) {
  // Gives borders and message to inputs with errors
...
function errorBox(errors, box) {
  // Outputs an error box
  if (errors.length > 0) {
    box.setMessage(errors);
  }
}

function successBox(form, box) {
  // Outputs success box and removes form
  form.remove();
  box.setMessage();
}
